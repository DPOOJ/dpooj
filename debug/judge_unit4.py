...
class Person():
    ...
    def add_credit(self, v:int):
        self.credit = min(self.credit + v, 20)
...
def judge_15(input_path, userout_path, stdout_path):
    input_lines = []
    output_lines = []
    ...
    def get_overdue_person_list(op:str) -> list:
        bad_list = []
        for p in all_persons:
            p:Person
            rm_book_list = []
            for b in p.book_credit:
                if b.due_date < global_date+timedelta(days=(op=='CLOSE')):
                    #print(global_date, p.get_id(), b.get_id())
                    bad_list.append(p)
                    rm_book_list.append(b)
            for b in rm_book_list:
                p.book_credit.remove(b)
    ...
    try:
        while idx_in < len(input_lines) and idx_out < len(output_lines):
            ...
            if op_in == 'OPEN' or op_in == 'CLOSE':
                ...
                bad_person_list = get_overdue_person_list(op_in)
                for bad_p in bad_person_list:
                    bad_p.add_credit(-2)
                ...
                for i in range(num_moves):
                    ...
                    to_list = get_list(to_out)
                    for_out = "*" if from_out == "ao" else for_out
                    if from_out == "ao" and to_out == "bs":
                        _, _, bad_sid = find_first_book_in_ao(ao, bookId_out)
                        bad_person = get_person(bad_sid)
                        bad_person.add_credit(-3)
                    book:Book = remove_book(from_list, bookId_out, for_out if from_out == 'ao' else "", all_persons if from_out == 'ao' else None)
                    if book is None:
                        return error(input_line,output_line,"can't find book in the place moved from")
                    if to_out == "ao" and find_book_in(person.ordered, bookId_out) is None:
                    ...
                    if b_count.get(bookId_out) is not None:
                        if b_count.get(bookId_out) >= 2:
                            ...
                            donor = get_person(donate_records.get(bookId_out)) 
                ...
                values_out = output_line.split(" ")
                if bookId_in == "credit":
                    [date_out, sid_out, credit_out] = values_out
                    date_out = date.fromisoformat(date_out.strip('[').strip(']'))
                    credit_out = int(credit_out)
                    if sid_out != sid_in:
                        return error(input_line,output_line,f"wrong student id")
                    person = get_person(sid_out)
                    if credit_out != person.credit:
                        return error(input_line,output_line,f"wrong output credit: got {credit_out}, expected {person.credit}")
                else:
                    [date_out, bookId_out, num_out] = values_out
                    date_out = date.fromisoformat(date_out.strip('[').strip(']'))
                    num_out = int(num_out)
                ...
                if op_in == "borrowed":
                    ...
                    if book is None:
                        ...
                        if book.type == "BU" or book.type == "CU":
                            remove_book(bdc, bookId_in)
                        #print(f"{bookId_in} borrowed?")
                        if result_out == 'reject' and not (book.type == "A" or book.type == "AU"):
                            add_book(bro, global_date, bookId_in)
                        if (book.type == "B" and have_any_book_type("B",person.held)) or (book.type == "BU" and have_any_book_type("BU",person.held)):
                            if result_out == "accept":
                                return error(input_line,output_line,f"can't borrow, the student already has type B book")
                            ...
                            if result_out == "accept":
                                if person.credit < 0:
                                    return error(input_line,output_line,f"can't borrow, the student's credit: {person.credit} < 0'")
                                add_book(person.held, global_date, bookId_in)
                                add_book(person.book_credit, global_date, bookId_in)
                            elif not (book.type == "A" or book.type == "AU") and person.credit >= 0:
                                return error(input_line,output_line,f"the student should be allowed to borrow this book")
                    ...
                    returned_book = remove_book(person.held, bookId_in)
                    remove_book(person.book_credit, bookId_in)
                    ...
                    if result_out == "accept":
                        ...
                        if btype == "A":
                            return error(input_line,output_line,f"can't order type A book")
                        if btype == "B":
                            if have_any_book_type("B",person.held):
                                return error(input_line,output_line,f"can't order, the student already has type B book")
                            if have_any_book_type("B",person.ordered):
                                return error(input_line,output_line,f"can't order, the student already ordered type B book")
                        elif btype == "C":
                            if have_book(person.held, bookId_in):
                                return error(input_line,output_line,f"can't order, the student already has the same type C book")
                            if have_book(person.ordered, bookId_in):
                                return error(input_line,output_line,f"can't order, the student already ordered the same type C book")
                        if person.credit < 0:
                                    return error(input_line,output_line,f"can't order, the student's credit: {person.credit} < 0'")
                        add_book(person.ordered, global_date, bookId_in)
                    ...
                    book = find_book_in(person.held, bookId_in)
                    c_book = find_book_in(person.book_credit, bookId_in)
                    if book is None:
                        return error(input_line,output_line,f"student can only renew book he/she holds")
                    if not book.is_formal():
                        if result_out == 'accept':
                        ...
                        if global_date <= renew_low or global_date > renew_high:
                            if result_out == 'accept':
                                if person.credit < 0:
                                    return error(input_line,output_line,f"can't renew, the student's credit: {person.credit} < 0'")
                                return error(input_line,output_line,f"not in renewable days: ({renew_low}, {renew_high}]")
                        else:
                            if result_out == 'reject':
                                if order_ok and person.credit >= 0:
                                    return error(input_line,output_line,f"the book should be allowed to renew in ({renew_low}, {renew_high}]")
                            else:
                                book.due_date += timedelta(days=30)
                                c_book.due_date += timedelta(days=30)
                    ...
                    add_book(all_books, global_date, bookId_in)
                    b_count[bookId_in] = 0
                    donate_records[bookId_in] = sid_in
                    person.add_credit(2)
...
if __name__ == "__main__":
    ...
    rets = judge_15(input_path,output_path,"")
    print(rets)
    sys.exit(rets[0])